...
<script>
  ...
  export default {
    ...
    mounted () {
        // Vue selection is getting messed up, skipping over the actual child which is messing up the
        // selection math, so we enforce that we use this.$el.children instead of this.$children
        // NOTE: also pretty sure this shouldn't only be in mounted - want to change this when the children change...
        Array.from(this.$el.children).forEach((child) => {
            d3.select(child).on('click', (event) => {
