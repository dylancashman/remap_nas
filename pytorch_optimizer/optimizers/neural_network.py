...
# Local imports
import graph_utils
from general_utils import get_nonzero_indices_in_vector, reorder_list_or_array, \
...
class NeuralNetwork(object):
  ...
  def _set_up(self):
    ...
    self.internal_layer_idxs = np.delete(list(range(self.num_layers)), [self.get_ip_layer_idx(),
  ...
  def get_edges(self):
    ...
    return list(self.conn_mat.keys())
    ...
    for lg in self._bkwd_ip_fwd_op_dist_type_order:
      ...
      for plt in self._path_length_type_order:
        ...
        self._bkwd_dists_to_ip[curr_key] = graph_utils.compute_nn_path_lengths(
          curr_edge_weights_T, list(reversed(list(range(self.num_layers)))), plt)
        self._fwd_dists_to_op[curr_key] = graph_utils.compute_nn_path_lengths(
          curr_edge_weights, list((list(range(self.num_layers)))), plt)
...
def compute_num_channels_at_each_layer(nn):
  ...
  # print("nn.layer_labels is ", nn.layer_labels)
  for layer_idx in range(1, nn.num_layers-1):
    ...
    # print("num_channels_out_of_each_layer is ", num_channels_out_of_each_layer)
    # print("channels_from_parents is ", channels_from_parents)
    # print("[type(x) for x in channels_from_parents] is ", [type(x) for x in channels_from_parents])
    # print("curr_parents is ", curr_parents)
    # print("layer_idx is ", layer_idx)
    # Determine the number of channels in
    curr_in = sum(channels_from_parents)
    # Determine the number of channels out
    if is_a_pooling_layer_label(ll) or ll in ['dropout', 'tanh', 'sigmoid', 'relu']:
      curr_out = curr_in
...
class ConvNeuralNetwork(NeuralNetwork):
  ...
  def _check_image_size_consistency(self):
    ...
    for layer_idx in range(1, self.num_layers):
      ...
      print(("layer_label is ", self.layer_labels[layer_idx]))
      print(("is_a_conv_layer_label(self.layer_labels[layer_idx] is ", is_a_conv_layer_label(self.layer_labels[layer_idx])))
      print(("is_a_pooling_layer_label(self.layer_labels[layer_idx] is ", is_a_pooling_layer_label(self.layer_labels[layer_idx])))
      ...
      print(("curr_parents is ", curr_parents))
      print(("parent_post_img_sizes is ", parent_post_img_sizes))
