...
import numpy as np
# Local imports
from kernel import ExpSumOfDistsKernel, SumOfExpSumOfDistsKernel
import neural_network
from oper_utils import opt_transport
  ...
  def evaluate(self, X1, X2, *args, **kwargs):
    ...
    if isinstance(X1, neural_network.NeuralNetwork) and \
      ...
      for i, x1 in enumerate(X1):
        X2_idxs = list(range(i, n2)) if x1_is_x2 else list(range(n2))
        for j in X2_idxs:
          x2 = X2[j]
